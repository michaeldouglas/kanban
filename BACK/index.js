...
socketIO.on("connection", (socket) => {
    ...
    socket.on("createTask", () => {
        socket.emit("tasks", cards);
    });

    socket.on("taskDragged", () => {
        socket.emit("tasks", cards);
    });

    socket.on("addComment", (data) => {
        let taskItems = cards.find(x => x.id === data.id);

        if (!taskItems.hasOwnProperty("comments")) {
            taskItems['comments'] = []
        }

        taskItems.comments.push({
            name: data.userId,
            text: data.comment,
            id: Math.random().toString(36).substring(2, 10),
        });

        socket.emit("comments", taskItems.comments);
    });

    socket.on("fetchComments", (data) => {
        const taskItems = cards.find(x => x.id === data.id);
        if (!taskItems.hasOwnProperty("comments")) {
            taskItems['comments'] = []
        }
        socket.emit("comments", taskItems.comments);
